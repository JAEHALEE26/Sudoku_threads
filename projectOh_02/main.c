...
void check_sudoku(void)
{
    ...
    thr_id = pthread_create(&p_thread[0], NULL, check_rows, NULL);
    if (thr_id < 0)
    {
     ...
     */
    thr_id = pthread_create(&p_thread[1], NULL, check_columns, NULL);
    if (thr_id < 0)
    {
        perror("thread create error : ");
        exit(0);
    ...
    struct thread_args p[9] = {{0, 0, 0}, {0, 3, 1}, {0, 6, 2}, {3, 0, 3}, {3, 3, 4}, {3, 6, 5}, {6, 0, 6}, {6, 3, 7}, {6, 6, 8}};

    for (int thr = 0; thr < 9; thr++){
        thr_id = pthread_create(&p_thread[thr + 2], NULL, check_subgrid, (void*)&(p[thr]));
        if (thr_id < 0)
        {
            perror("thread create error : ");
            exit(0);
    ...
    {
        pthread_join(p_thread[end], (void *)&result);
    }
    /*
     * 각 행에 대한 검증 결과를 출력한다.
     */
